    ...
    def place_models(self, controller, cluster_env: ClusterEnv,
        ...
        if self.verbose >= 1:
            ...
            if not self.ensemble:
                print(f"{self.name} solver time: {solver_time:.2f} s")
            else:
                print(f"{debug_info['policy']} solver time: {solver_time:.2f} s")

        placement.verify(model_datas, cluster_env)
        extra_info = debug_info['policy'] if self.ensemble else None
        # if self.ensemble:
        #     placement = (placement, debug_info['policy'])
        # else:
        #     placement = (placement, None)
        return placement, extra_info
...
class PlacementEvaluator:
    ...
    def get_goodput_simulation(sol: ModelPlacement,
        ...
        if fast_simulator:
            stats, _, _ = approximate_one_case(serving_case, fast_stats=True)
        else:
