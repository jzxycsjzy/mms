...
def approximate_one_case(case: ServingCase,
    ...
    if fast_stats:
        ...
        try:
            print(f"placement: {placement}, Branch choice: {isinstance(placement, ModelPlacement)}, start: {start}")
        except:
            print(f"placement: {placement}, Branch choice: {isinstance(placement, ModelPlacement)}, start is not defined.")
        stats = workload.compute_stats(start, finish, good, warmup)
        stats.group_num_requests = tuple(group_num_requests)
    return stats, placement, ensemble
