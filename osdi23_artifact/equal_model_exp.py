...
from alpa_serve.placement_policy import DSScheduler
...
if __name__ == "__main__":
    ...
    if model_type == "bert-103.5b":
        ...
        if args.workload == "azure_v2":
            # MARK - branch: azure_v2 in this branch
            policies = ["sr-greedy", "sr-replace-21600", "mp-search"]
        else:
            # MARK - branch: azure_v1 in this branch
    ...
    if args.ensemble:
        policy = DSScheduler()
        policy.ensemble = True
        for policy_n in policies:
            policy.add_new_policy(policy_n)
        ray.get(policy.policy_actor.load_status.remote())
    ...
    if "goodput_vs_num_devices" in experiments:
        ...
        for num_devices in num_devices_list:
            if args.ensemble:
                """
                MARK: When using ensembel mode, change the logic.
                """
                cases.append(EqualModelCase(exp_name,
                        num_devices, mem_budget, model_type, fixed_num_models,
                        total_rate, rate_distribution,
                        arrival_process, arrival_process_kwargs,
                        fixed_slo_scale, duration, policies, None, None, None, None, policy))
            else:
                for policy_name in policies:
                    cases.append(EqualModelCase(exp_name,
                        ...
                        total_rate, rate_distribution,
                        arrival_process, arrival_process_kwargs,
                        fixed_slo_scale, duration, policy_name, None, None, None, None, None))
            ...
            if args.ensemble:
                ...
                for policy_name in policies:
                    cases.append(EqualModelCase(exp_name,
                        ...
                        total_rate, rate_distribution,
                        arrival_process, arrival_process_kwargs,
                        slo_scale, duration, policy_name, None, None, None, None, None))
    ...
    for i in range(M):
        ...
        result = run_equal_model_cases(cases[start_case:end_case],
    ...
    if args.ensemble:
        ray.get(policy.policy_actor.save_status.remote())
    # ### model vs devices ###
