...
from alpa_serve.placement_policy import (ClusterEnv, ModelData,
    ...
    ModelParallelismEqual, DSScheduler)
...
EqualModelCase = namedtuple("EqualModelCase", [
    ...
    "test_start", "test_end", "ensemble"])

def get_equal_model_serving_case(case, prof_database=None):
    if prof_database is None:
        prof_database = ProfilingDatabase("/home/rongyuan/WorkSpace/DLInference/mms/alpa_serve/profiling_result.pkl")

    (exp_name, num_devices, mem_budget, model_type, num_models,
     total_rate, rate_distribution, arrival_process, arrival_process_kwargs,
     slo_scale, duration, policy_name, train_start, train_end, test_start, test_end, ensemble) = case
    ...
    def place_models(controller):
        ...
        if ensemble is None:
            ...
            placement = ensemble.place_models(controller, cluster_env, model_datas, train_workload)
        return placement
...
_DATA_HEADS = ("exp_name",
               ...
               "slo_scale", "duration", "policy_name", "train_start", "train_end", "test_start", "test_end", "ensemble",
...
def run_one_equal_model_case(case, mode,
    ...
    # MARK: remote server function entrance.
    serving_case = get_equal_model_serving_case(case, prof_database)
    if mode == "simulate":
        stats, placement, ensemble = approximate_one_case(serving_case, debug=debug, enable_batching=enable_batching)
    else:
    ...
    p90, goodput = Workload.print_stats(stats)    
    ...
    if ensemble is not None:
        ...
        print(f"cases: {case}")
        print(f"num_devices: {case.num_devices}, mem_budget: {case.mem_budget}, model_type: {case.model_type}, num_models: {case.num_models}, arrival_process: {case.arrival_process}, rate_scale: {case.arrival_process_kwargs['rate_scale']}, cv_scale: {case.arrival_process_kwargs['cv_scale']}, slo_scale: {case.slo_scale}, policy: {ensemble}")
        print(f"group #req: {stats.group_num_requests}")
        """
        Update the tree in DSScheduler
        """
        # data = [ensemble, case.arrival_process, case.model_type, case.num_devices, case.mem_budget, case.num_models, p90, goodput]
        data = [ensemble, "test", "test", case.num_devices, case.mem_budget, case.num_models, p90, goodput]
        case.ensemble.update_data(data)
...
def run_equal_model_cases(cases, output_file=None,
    ...
    if parallel and not ray.is_initialized():
        ray.init(address="auto", namespace="alpa_serve",
                 ...
                 "excludes": ["backup"]})
    # MARK: it it the entrance of the whole workflow
    if parallel:
...
def read_equal_model_case_tsv(filename):
    ...
    for line in open(filename):
        ...
        try:
            ...
            ensemble, placement, goodput, mode) = line.split("\t")
        except Exception:
            (exp_name,
            num_devices, mem_budget, model_type, num_models,
            total_rate, rate_distribution,
